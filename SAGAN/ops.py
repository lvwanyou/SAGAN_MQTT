...
def multihead_attention(queries, keys, values,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        # Linear projections
        Q = tf.layers.dense(queries, d_model, use_bias=True)  # (N, num_steps, d_model)
        K = tf.layers.dense(keys, d_model, use_bias=True)  # (N, num_steps, d_model)
        V = tf.layers.dense(values, d_model, use_bias=True)  # (N, num_steps, d_model)
